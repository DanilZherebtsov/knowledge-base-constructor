...
- **Запустить:** склонируй [публичное зеркало](https://github.com/DanilZherebtsov/knowledge-base-constructor) в пустую папку, открой её в Claude Code или Claude Cowork и просто начни разговор — конструктор интервью соберёт проект (на выходе — заполненный `CLAUDE.md` и структура вики; в конце Claude попросит разрешение убрать временные файлы конструктора: в Claude Desktop появится окно — разреши, а если окна нет, Claude скажет, что переключить). См. [START-HERE.md](START-HERE.md).
